...
- **Solubility (溶解度)**：近似正態分布，集中在 70-90 g/L
- **BoilingPoint (沸點)**：集中在 60-150°C，少數高沸點溶劑 (> 180°C)
- **Viscosity (黏度)**：右偏分布，大多 < 1 cP，少數高黏度溶劑 (> 2 cP)
- **Density (密度)**：正態分布，集中在 0.8-0.9 g/cm³
- **Toxicity (毒性)**：離散分布，集中在 2-6 級，少數高毒性溶劑 (級別 8-10)
- **Cost (成本)**：右偏分布，多數 < 3 $/kg，少數高成本溶劑 (> 5 $/kg)
- **Polarity (極性)**：近似均勻分布，涵蓋 0-10 範圍
- **DielectricConstant (介電常數)**：正態分布，集中在 5-15
- **RefractiveIndex (折射率)**：正態分布，集中在 1.35-1.45
- **FlashPoint (閃點)**：寬分布，20-100°C 範圍

**關鍵發現**：

- ✓ **黏度與成本呈右偏分布**：大多數溶劑具有低黏度和低成本，少數例外
- ✓ **毒性為離散分布**：存在明顯的級別分組（2, 4, 6 級較常見）
- ✓ **物化性質範圍合理**：所有特徵值都在實際溶劑的合理範圍內
- ✓ **少數異常值**：需在後續分析中特別關注極端值溶劑
