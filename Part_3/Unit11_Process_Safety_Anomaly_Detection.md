# Unit11｜製程安全與異常偵測：Isolation Forest + PCA/MSPC（$T^2$ / SPE）

**課程名稱**：化工資料科學與機器學習實務（CHE-AI-101）  
**Part 3 導讀**：在工廠裡，「標記故障」通常很貴、很慢、甚至根本沒有。因此本單元以 **非監督式異常偵測 + 降維監控** 為主軸：先偵測「不對勁」，再逐步逼近「可能原因」。

> 本單元內容主要由舊版 `Part_3/old/Unit07_Process_Safety.*` 搬移重整，並補上「可上線」所需的告警/漂移/重訓思維。
...
## 4. 實戰演練：`Part_3/Unit11_Process_Safety_Anomaly_Detection.ipynb`
...
![單變數壓力異常偵測（原始）](Unit11_Results/01_pressure_iforest_raw.png)
...
![異常分數時間序列](Unit11_Results/02_score_raw.png)
...
![引入滾動特徵的異常檢測](Unit11_Results/03_pressure_iforest_rolling.png)
...
![MSPC 雙統計量監控圖](Unit11_Results/04_mspc_t2_spe.png)
...
當 SPE 超標時，我們需要知道「是哪些變數導致的」。以下為異常期間（t=1800-2000）SPE 均值最高的 8 個變數：

![SPE 貢獻圖](Unit11_Results/05_spe_contribution.png)
...
![告警邏輯示意圖](Unit11_Results/07_alarm_logic_spe.png)
...
![EWMA 監控圖](Unit11_Results/08_ewma_spe.png)
...
![CUSUM 監控圖](Unit11_Results/09_cusum_spe.png)
...
![OOD Gate 檢測結果](Unit11_Results/10_ood_gate_zcount.png)
...
## 7. 進階挑戰：Tennessee Eastman Process (TEP) 數據集

### 7.1 TEP 簡介

**Tennessee Eastman Process (TEP)** 是化工製程監控領域最經典的 benchmark 數據集，由 Eastman Chemical Company 於 1993 年發布。

**製程特性**：
- **52 個測量變數**：溫度、壓力、流量、成分等
- **12 個控制變數**：閥門開度、加熱器功率等
- **21 種故障模式**：涵蓋感測器故障、反應異常、冷卻器結垢等
...
**數據位置**：
本專案倉庫中已包含教材示例用的 `Jupyter_Scripts/Unit07_Results/TEP_data.zip`，Notebook 會優先讀取本地檔；若不存在才會提示下載。

**典型分析流程**：
1. **選擇正常數據**：`d00.dat`（無故障的訓練數據）
2. **選擇異常數據**：`d01_te.dat` - `d21_te.dat`（21 種故障）
3. **建立 PCA/MSPC 模型**：僅用正常數據訓練
4. **測試檢測能力**：對每種故障計算檢測延遲 (Detection Delay) 和檢測率 (Detection Rate)

**評估指標**：
- **FAR (False Alarm Rate)**：正常數據被誤判為異常的比例（應 < 5%）
- **FDR (Fault Detection Rate)**：異常數據被正確檢測的比例（應 > 90%）
- **DD (Detection Delay)**：從故障注入到首次檢測的時間（應 < 10 個採樣週期）

### 7.3 TEP 挑戰與學習重點

**挑戰 1：選擇合適的變數子集**
- 52 個變數中有些是冗餘的（如多個溫度計測量同一設備）
- 需要領域知識篩選關鍵變數（或使用變數選擇演算法）

**挑戰 2：決定 PCA 主成分數量**
- 保留太少：丟失重要信息，漏報率高
- 保留太多：包含雜訊，誤報率高
- 建議：嘗試不同的累積變異解釋比例（85%-95%）

**挑戰 3：處理動態性**
- TEP 是動態製程，當前狀態依賴歷史狀態
- 解決方案：
  - 使用 Dynamic PCA (DPCA)
  - 引入 lag variables（時間延遲變數）
  - 配合 HMM/GMM 進行狀態分段（參見 Unit12）

**學習目標**：
- 掌握高維數據的降維與監控策略
- 理解不同故障模式對 $T^2$ 和 SPE 的影響
- 學習調整模型參數以平衡檢測率和誤報率
...
| 方法 | 類型 | 檢測目標 | 計算複雜度 | 可解釋性 | 適用場景 |
| :--- | :--- | :--- | :---: | :---: | :--- |
| **Isolation Forest** | 非監督 | 全局異常點 | 低 | 中 | 未知故障、快速掃描 |
| **PCA/MSPC ($T^2$)** | 非監督 | 極端但合理操作 | 低 | 高 | 操作窗口監控 |
| **PCA/MSPC (SPE)** | 非監督 | 變數關聯破壞 | 低 | 高 | 感測器故障、設備異常 |
| **EWMA** | 統計 | 緩慢漂移 | 低 | 高 | 結垢、老化、催化劑失活 |
| **CUSUM** | 統計 | 微小偏移 | 低 | 高 | 感測器零點偏移 |
| **OOD Gate** | 統計 | 輸入分佈偏移 | 低 | 高 | 原料切換、設備改造 |
...
**經驗 2：多層次防護**
不要依賴單一方法，建議配置：
- **Layer 1**：OOD Gate（輸入驗證）
- **Layer 2**：Isolation Forest（全局掃描）
- **Layer 3**：PCA/MSPC（關聯監控）
- **Layer 4**：EWMA/CUSUM（趨勢預警）

**經驗 3：可解釋性 > 精確度**
在工業現場，「能向操作員解釋為什麼告警」比「高檢測率」更重要。
- 貢獻圖：指出哪些變數異常
- 趨勢圖：顯示異常如何發展
- 歷史對比：與過去類似事件對比
...
### 9.1 與 Unit08（軟感測器）的關聯

**相同點**：
- 都需要監控模型殘差（Unit08 的 `residual = y_true - y_pred`）
- 都需要設定閾值與告警機制
- 都需要處理輸入漂移（Input Monitoring）
...
**整合方案**：
在軟感測器（Unit08）中加入 MSPC 監控：
1. 監控 **輸入特徵的 SPE**（檢測輸入分佈偏移）
2. 監控 **預測殘差**（檢測模型性能退化）
3. 兩者結合判斷是「輸入問題」還是「模型問題」

### 9.2 與 Unit12（進階深度學習）的關聯

**Unit12 預告**：
- **Autoencoder**：用深度學習做非線性 PCA
  - 更強的特徵提取能力（捕捉非線性關聯）
  - 重建誤差（Reconstruction Error）類似 SPE
- **Variational Autoencoder (VAE)**：同時學習數據的均值和變異
  - 可用於生成「正常數據」樣本（數據擴增）
  - Latent space 的 KL 散度可用於 OOD 檢測

**整合方向**：
- PCA/MSPC：適合線性關聯、低維數據
- Autoencoder：適合非線性關聯、高維數據
- 實務上可先用 PCA 快速驗證，再用 Autoencoder 精細化

### 9.3 與 Unit13（時間序列預測）的關聯

**時間序列視角的異常檢測**：
1. 訓練時間序列預測模型（如 LSTM）
2. 計算預測誤差：$e_t = x_t - \hat{x}_t$
3. 監控預測誤差的 EWMA/CUSUM
...
**[Next Unit]**  
Unit12 進入「狀態切換」：用 GMM/HMM（或等效的動態分段）把批次反應/開停機流程拆成可解釋的階段，讓監控與 SOP 更容易落地。同時探索 Autoencoder 等深度學習方法在異常檢測中的應用。

---

## 附錄 A：執行結果完整清單

本單元 Notebook 執行後生成以下結果檔案（位於 `Unit11_Results/`）：
...
**下一步**：前往 Unit12 學習進階深度學習方法（Autoencoder、VAE）在製程監控中的應用！
