...
- **演算法原理**：
  ...
  - 損失函數： $L(\mathbf{w}) = \text{MSE} + \alpha \rho \sum_{i=1}^{n} |w_i| + \alpha (1-\rho) \sum_{i=1}^{n} w_i^2$
...
- **決定係數 ( $R^2$ )**： $1 - \frac{\sum(y_i - \hat{y}_i)^2}{\sum(y_i - \bar{y})^2}$ ，表示模型解釋變異的比例 (0-1)
- **平均絕對百分比誤差 (MAPE)**： $\frac{100\%}{n}\sum\frac{|y_i - \hat{y}_i|}{|y_i|}$ ，相對誤差百分比

### 化工實務評估考量
- **預測準確度**：滿足工業要求的精度 (如 $\pm 2\%$ )
- **模型穩定性**：不同批次資料上的表現一致性
- **計算成本**：即時系統的響應時間要求
- **可維護性**：模型更新與參數調整的難易度
