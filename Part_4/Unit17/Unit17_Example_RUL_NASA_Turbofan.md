...
- 訓練集：100 台引擎，完整 run-to-failure 數據
- 測試集：100 台引擎，在隨機時刻截斷
- RUL 分布廣泛，涵蓋早期到臨界階段
...
- 不同引擎的退化模式相似但有個體差異
- 部分感測器（如 sensor_2, sensor_3）顯示明顯退化趨勢
- 時序特徵對 RUL 預測至關重要
...
在預知保養中，**低估 RUL**（早期預測故障）比**高估 RUL**（晚期預測故障）的代價更低。
...
**詳細改進策略與實施計畫請參考：**
- 📄 **完整分析報告**：[Model_Performance_Analysis_Report.md](Model_Performance_Analysis_Report.md)（27頁）
- 📓 **快速實施手冊**：[Unit17_Model_Improvement_Quick_Fixes.ipynb](Unit17_Model_Improvement_Quick_Fixes.ipynb)（30分鐘可完成）
- 📋 **執行摘要**：[README_Performance_Review.md](README_Performance_Review.md)（一頁總覽）
...
| 📊 **本月** | 超參數優化 | 280-320 | 4 hr | ⭐⭐⭐ | ⭐⭐⭐ |
| 🎓 **長期** | 注意力機制 | 250-300 | 8 hr | ⭐⭐⭐⭐ | ⭐⭐ |
| 🔬 **研究** | 多任務學習 | 200-250 | 2 days | ⭐⭐⭐⭐⭐ | ⭐ |

**推薦實施順序：**
1. **第 1 天**：執行「偏差校正 + 集成」（30 分鐘，零風險，NASA Score → 350-400）
2. **第 1 週**：若不滿意，進行「非對稱損失重訓練」（目標 < 350）
3. **第 1 月**：持續優化超參數和架構（追求 < 300）
4. **長期**：研究 Transformer、多任務學習（學術興趣或極致性能需求）
