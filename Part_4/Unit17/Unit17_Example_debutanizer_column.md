...
**統計特性：**
- 目標變數（ $y\_\mathrm{C4Content}$ ）變異範圍：0.0 ~ 1.0（標準化後）
- 所有輸入變數均已標準化
...
1. **特徵與目標的相關性極弱**：
   - 所有特徵與 $y\_\mathrm{C4Content}$ 的相關性絕對值 < 0.25
   - 最強的相關變數（ $u4\_\mathrm{FlowToNext}$ ）也僅 0.15
   - 通常需要 $|r| > 0.3$ 才算有中等相關性
...
> **預測性能上限**：當特徵相關性如此弱時，即使是最複雜的模型，其 $R^2$ 也很難超過 0.3-0.4。
...
| 數據集 | R² | MSE | RMSE | MAE |
|--------|-----|-----|------|-----|
| **訓練集** | 0.7426 | 0.0018 | 0.0422 | 0.0298 |
| **驗證集** | 0.3825 | 0.0045 | 0.0671 | 0.0513 |
...
| 數據集 | R² | MSE | RMSE | MAE |
|--------|-----|-----|------|-----|
| **訓練集** | 0.8418 | 0.0011 | 0.0333 | 0.0235 |
| **驗證集** | 0.5903 | 0.0030 | 0.0547 | 0.0398 |
...
3. **性能天花板極低**：
   - 最佳模型 Random Forest 的 Test $R^2$ = 0.2
   - 只能解釋 20% 的方差
   - 80% 的變異無法被任何模型解釋
...
$$
y_{\mathrm{C4Content}} = f(\text{進料組成}, \text{操作條件}, \text{塔結構})
$$
