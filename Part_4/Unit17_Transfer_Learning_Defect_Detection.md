...
> **教學目標**：本單元延續 Unit16 的內容，展示如何使用遷移學習（Transfer Learning）技術，將在 ImageNet 上預訓練的 MobileNetV2 模型應用於工業缺陷檢測任務。透過對比傳統機器學習方法（Random Forest、MLP）與深度學習方法的性能差異，深入理解遷移學習在小樣本數據集上的優勢。
...
4. **對比學習**：量化分析 Baseline（sklearn）vs. 遷移學習（MobileNetV2）的性能差異
...
## 第三章：數據探索與Baseline模型（承接Unit16）

### 3.1 NEU-DET 數據集特性

**數據載入與預處理**：

對於 Baseline 模型（Random Forest 和 MLP），使用較小的 64×64 解析度以降低計算成本：

```python
def load_image_folder(root_folder, img_height=64, img_width=64, color_mode='rgb'):
    """
    參數保留 [0, 255] 範圍，不進行歸一化
    適合傳統機器學習方法（Random Forest, MLP）
    """
```

**特徵維度**：
- 單張影像：64×64×3 = 12,288 features
- 訓練集：1440 samples × 12,288 features
- 驗證集：360 samples × 12,288 features
...
![Defect Samples](outputs/P4_Unit17_Results/00_defect_samples.png)
...
### 3.3 Baseline 模型：Random Forest 與 MLP
...
![Confusion Matrix - Baseline](outputs/P4_Unit17_Results/01_confusion_matrix_baseline.png)

**Random Forest 結果**：
- **Validation Accuracy**: ~83.6%
- **主要混淆**：Patches ↔ Pitted Surface
...
混淆矩陣 $\mathbf{C}$ 中，元素 $C_{ij}$ 表示真實類別為 $i$ 但預測為 $j$ 的樣本數。
...
**MLP 主要困難**：
- **Patches** 容易誤判為其他類別（特徵模糊）
- **Pitted Surface** 與 **Rolled-in Scale** 有一定混淆（紋理相似）
...
![Learning Curves](outputs/P4_Unit17_Results/09_mobilenetv2_learning_curves.png)
...
![Confusion Matrix - MobileNetV2](outputs/P4_Unit17_Results/11_mobilenetv2_confusion_matrix.png)
...
|-------|----------|-----------|--------|----------|
| Random Forest | 83.6% | 0.836 | 0.836 | 0.836 |
| MLP | 86.4% | 0.864 | 0.864 | 0.864 |
| **MobileNetV2** | **100.0%** | **1.000** | **1.000** | **1.000** |

**提升幅度**：
- 相對於 Random Forest：+16.4% accuracy
- 相對於 MLP：+13.6% accuracy

### 6.2 預測結果可視化

![Prediction Visualization](outputs/P4_Unit17_Results/13_mobilenetv2_predictions.png)
...
![Model Comparison](outputs/P4_Unit17_Results/12_model_comparison.png)
...
|-------|--------------|----------|
| Random Forest | ~30 seconds | CPU |
| MLP | ~2 minutes | CPU |
| **MobileNetV2** | **~15 minutes** | **GPU (T4/P100)** |

**時間複雜度分析**：
- Random Forest: $O(n \cdot \log n \cdot d \cdot T)$，其中 $T=100$ 棵樹
- MLP: $O(n \cdot d \cdot h \cdot \text{epochs})$
- MobileNetV2: $O(n \cdot H \cdot W \cdot C \cdot K^2 \cdot \text{epochs})$

儘管訓練時間較長，但MobileNetV2只需訓練一次即可達到完美性能。

#### 3. Model Size（模型大小）

| Model | Parameters | Disk Size |
|-------|-----------|-----------|
| Random Forest | ~100K (100 trees) | ~15 MB |
| MLP | ~1.6M | ~6 MB |
| **MobileNetV2** | **~3.5M** | **~14 MB** |

**參數效率**：
- MobileNetV2 使用深度可分離卷積，相比標準CNN減少 8-9 倍參數
- 適合邊緣設備部署（Raspberry Pi, Jetson Nano）

#### 4. Inference Speed（推論速度）

| Model | Inference Time (per image) | FPS |
|-------|---------------------------|-----|
| Random Forest | ~5 ms | 200 |
| MLP | ~3 ms | 333 |
| **MobileNetV2** | **~15 ms (GPU)** | **~67** |
| **MobileNetV2** | **~50 ms (CPU)** | **~20** |
...
|-------|----------|-------------|---------|-------|
| Random Forest | ❌ Sensitive | ❌ Sensitive | ❌ Sensitive | ⚠️ Moderate |
| MLP | ❌ Sensitive | ❌ Sensitive | ❌ Sensitive | ⚠️ Moderate |
| **MobileNetV2** | **✓ Robust** | **✓ Robust** | **✓ Robust** | **✓ Robust** |
...
|-------|-------------------|-------------------|---------------|
| Random Forest | ✓ Available | ❌ No | ✓ Tree paths |
| MLP | ⚠️ Difficult | ❌ No | ❌ Black box |
| **MobileNetV2** | ⚠️ Difficult | **✓ Grad-CAM** | ❌ Black box |
...
**1. 源域與目標域的相似性**：
雖然 ImageNet（自然物體）與 NEU-DET（鋼材缺陷）看似不同，但底層視覺特徵（邊緣、紋理）高度相關：
...
| 指標 | Baseline (MLP) | MobileNetV2 | 提升幅度 |
|------|---------------|-------------|---------|
| Validation Accuracy | 86.4% | **100.0%** | +13.6% |
| F1-score (macro) | 0.864 | **1.000** | +13.6% |
| 最差類別 Recall | 0.78 (Patches) | **1.00** | +22% |
| 訓練時間 | 2 min (CPU) | 15 min (GPU) | +650% |
| 推論速度 (CPU) | 3 ms | 50 ms | +1567% |
| 模型大小 | 6 MB | 14 MB | +133% |

**關鍵洞察**：

1. **遷移學習的威力**：
   - ImageNet 預訓練特徵在工業缺陷檢測中高度有效
   - 僅需 1440 張訓練影像即可達到完美分類

2. **兩階段訓練的必要性**：
   - Stage 1 快速適應（8 epochs → 99.4% acc）
   - Stage 2 精細調優（3 epochs → 100% acc）
   - 小學習率微調避免災難性遺忘
...
```
[產線影像擷取] → [預處理] → [MobileNetV2推論] → [決策邏輯] → [控制動作]
        ↓              ↓             ↓              ↓            ↓
...
| 信心度區間 | 決策 | 動作 | 比例 |
|-----------|-----|------|------|
| > 0.95 | 自動分類 | 直接放行/剔除 | 85% |
| 0.60 - 0.95 | 人工複檢 | 進入審查隊列 | 12% |
| < 0.60 | 低信心 | 停線檢查 | 3% |
